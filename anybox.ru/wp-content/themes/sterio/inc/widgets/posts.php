				...
				foreach($posts as $p) {?>
                    <li>
						<?php if ( ($instance['display_date']) ) { 
							$date=explode(':',get_the_time('F:d', $p->ID));
							$day=$date[1];
							$month=$date[0];
                        ...
                        <?php if ( $instance['display_title'] ) { ?> <a class="title" href="<?php echo get_permalink($p->ID); ?>" rel="bookmark" title="<?php echo $p->post_title; ?>"><?php echo $p->post_title?></a><?php } ?>
